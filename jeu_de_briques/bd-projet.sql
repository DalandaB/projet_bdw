...
--création de tables personnelle
SET search_path TO legos;
...
CREATE TABLE Construction (
    ...
    id_photos INT    
    --j'ai des doutes à partir d'ici
...
-- Etape 1 de suppression de la contrainte de clé
ALTER TABLE Tour DROP CONSTRAINT tour_pkey;

-- Étape 2 : Ajouter la nouvelle clé primaire avec id_joueuse inclus
ALTER TABLE Tour ADD PRIMARY KEY (numero_tour, id_partie, id_joueuse);

-- on retire les contraintes de NULLITE pour les deux colonnes non urgentes
-- retirer la contrainte de nullité sur la colonne piocher
ALTER TABLE tour ALTER COLUMN piocher DROP NOT NULL;

-- retirer la contrainte de nullité sur la colonne id
ALTER TABLE tour ALTER COLUMN id DROP NOT NULL;

--mettre les deux colonnes respectives à NULL
UPDATE tour SET piocher = NULL, id = NULL;
...
INSERT INTO jouer(id_joueuse,id_partie,gagnante,score_perso)
VALUES 
(2,1,'false',16),
(3,1,'false',17),
(1,2,'false',22),
...
UPDATE tour
SET piocher = CASE
    WHEN numero_tour % 2 = 0 THEN true
    ELSE false
END;

--  Insertion des id correspondant aux id de piece dans la table tour
-- Étape 1 : Créer une table temporaire avec des ID uniques
CREATE TEMP TABLE temp_ids AS
SELECT generate_series(1, 1280) AS id ORDER BY random();

-- Étape 2 : Mettre à jour la table "tour" avec ces ID
WITH cte AS (
    SELECT numero_tour, id_partie, id
    FROM tour
    JOIN temp_ids USING (id)
)
UPDATE tour
SET id = cte.id
FROM cte
WHERE tour.numero_tour = cte.numero_tour AND tour.id_partie = cte.id_partie;
